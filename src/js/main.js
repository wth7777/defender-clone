// ============================================================================
// Green Lantern: Ultima Odyssey - DEBUG VERSION WITH TOUCH VISUALIZATION
// ============================================================================
...
class UI {
    render(player) {
        ...
        // DEBUG INFO
        if (DEBUG.showCanvasInfo && gameState.debug.canvasRect) {
            ctx.fillStyle = '#0f0';
            ctx.font = '10px monospace';
            const rect = gameState.debug.canvasRect;
            ctx.fillText(`Canvas: ${rect.width}x${rect.height}`, 10, CONFIG.LOGICAL_HEIGHT - 60);
            ctx.fillText(`Scale: ${gameState.debug.scaleFactor.toFixed(2)}`, 10, CONFIG.LOGICAL_HEIGHT - 45);
            if (gameState.debug.lastTouch) {
                ctx.fillText(`Touch: (${gameState.debug.lastTouch.x},${gameState.debug.lastTouch.y})`, 10, CONFIG.LOGICAL_HEIGHT - 30);
                ctx.fillText(`Logical: (${gameState.debug.lastLogicalTouch.x.toFixed(0)},${gameState.debug.lastLogicalTouch.y.toFixed(0)})`, 10, CONFIG.LOGICAL_HEIGHT - 15);
            }
        }
    }
}
...
class InputHandler {
    ...
    setupListeners() {
        ...
        canvas.addEventListener('touchstart', (e) => {
            ...
            // DEBUG
            if (DEBUG.logTouchEvents) {
                console.log(`TOUCH START: client(${touch.clientX},${touch.clientY}) -> canvas(${x.toFixed(1)},${y.toFixed(1)}) logical`);
                gameState.debug.lastTouch = { x: touch.clientX, y: touch.clientY };
                gameState.debug.lastLogicalTouch = { x, y };
                gameState.debug.canvasRect = { width: rect.width, height: rect.height };
                gameState.debug.scaleFactor = CONFIG.SCALE;
            }
        }, { passive: false });
        ...
        canvas.addEventListener('touchend', (e) => {
            ...
            if (this.touchStartPos) {
                ...
                // DEBUG
                if (DEBUG.logTouchEvents) {
                    console.log(`TOUCH END: client(${touch.clientX},${touch.clientY}) -> canvas(${x.toFixed(1)},${y.toFixed(1)}) logical`);
                    gameState.debug.lastTouch = { x: touch.clientX, y: touch.clientY };
                    gameState.debug.lastLogicalTouch = { x, y };
                }
            }
...
// ============================================================================
function init() {
    // DEBUG: Alert removed for less intrusion
...
function gameLoop(timestamp) {
    ...
    // Update debug info
    if (DEBUG.showTouchCoords || DEBUG.showBounds || DEBUG.showCanvasInfo) {
        updateDebugInfo();
    }
